...
do
	...
	#echo $all_username
	echo $all_username >> username_list_5.txt

done < "username_result_5.txt"
...
then
	...
	done < "username_bucket_check_4.txt"
	echo "Your bucket name is $bucket_to_handle"
	echo "Delete the file"
	echo ""
	delete_file_in_bucket=`aws s3 rm s3://$bucket_to_handle/$file_to_delete`
	echo "Finish deleting"
	delete_file_3=`rm username_bucket_check_4.txt`
fi


delete_file_2=`rm username_list_5.txt`
